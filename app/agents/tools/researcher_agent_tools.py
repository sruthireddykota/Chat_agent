...
from app.utils.tools import mcp_web_search, hf_search, _get_history
...
class ResearcherTools:
    ...
    async def _get_history_logic(self) -> str:
        return await _get_history(
            mcp_client=self.mcp_manager.mcp_websearch_client,
            session_id=self.session_id,
        )

    async def _web_search_logic(self, query: str, freshness: str) -> Dict:
        key = _cache_key("web_search", query=query, freshness=freshness)
        try:
    ...
    async def _repo_search_logic(self, query: str) -> str:
        key = _cache_key("repo_search", query=query)
        try:
            return await self._cached_call(
                key,
    ...
    async def _papers_search_logic(self, query: str) -> str:
        key = _cache_key("papers_search", query=query)
        try:
            return await self._cached_call(
                key,
                lambda: hf_search(mcp_client=self.mcp_manager.mcp_huggingface_client, tool_name="paper_search", query=query),
            )
        except Exception as e:
    ...
    async def _spaces_search_logic(self, query: str) -> str:
        key = _cache_key("spaces_search", query=query)
        try:
            return await self._cached_call(
                key,
    ...
    async def _documentation_search_logic(self, query: str, product: str) -> str:
        key = _cache_key("documentation_search", query=query, product=product)
        try:
            return await self._cached_call(
                key,
